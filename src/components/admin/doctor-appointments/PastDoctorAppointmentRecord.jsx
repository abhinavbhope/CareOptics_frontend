...
import { Edit, Trash2, Calendar, Stethoscope, ChevronDown } from 'lucide-react';
import { AlertDialog, AlertDialogAction, AlertDialogCancel, AlertDialogContent, AlertDialogDescription, AlertDialogFooter, AlertDialogHeader, AlertDialogTitle, AlertDialogTrigger } from "@/components/ui/alert-dialog";
import { useToast } from '@/hooks/use-toast';
import { deletePastUserAppointment } from '@/lib/admin/doctorPastUserApi';
...
export function PastDoctorAppointmentRecord({ appointment, onEdit, onDeleteSuccess, pastUserId }) {
    const { toast } = useToast();
    const [isExpanded, setIsExpanded] = useState(false);

    const handleDelete = async () => {
        try {
            await deletePastUserAppointment(pastUserId, appointment.id);
                        ...
                        {isExpanded && (
                            ...
                            >
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                                    <div>
                                        <p className="font-semibold text-muted-foreground">Patient Name:</p>
